function load() {
	if(document.body.classList.contains('darkTheme')){
		...
		var map = new mapboxgl.Map({
		  container: 'map',
		  style: 'mapbox://styles/sofiavillazon/ckleralz95ayd18p6pnuxribe',
		  center: [-96, 37.8],
		  zoom: 2
		});
	}else{
	...
	geojson.features.forEach(function(marker) {
		...
		var randomColor = colorPin[Math.floor(Math.random()*colorPin.length)];
		var animation = " heart";
	
		var el = document.createElement('div');
		el.className = 'marker ' + randomColor ;
		// el.className = 'heart';
...
var toggle = function (elem) {
    ...
    document.getElementById("modalNews").classList.remove('show');
...
document.querySelectorAll('.btnScientific').forEach(item => {
	item.addEventListener('click', event => {
		modalScientifics.show();
	})
})

document.querySelectorAll('.ckickScientific').forEach(item => {
	item.addEventListener('click', event => {
		modalScientifics.show();
	})
})